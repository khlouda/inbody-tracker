...
function calculatePredictions(scans) {
  ...
  const daysFromFirst = (lastDate - firstDate) / (1000 * 60 * 60 * 24)
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(scans) || scans.length < 3) {
      return res.status(400).json({ success: false, error: 'At least 3 scans are required for predictions.' })
    }
...
Projected values at 4 weeks: ${JSON.stringify(predictions.weeks4, null, 2)}
Projected values at 8 weeks: ${JSON.stringify(predictions.weeks8, null, 2)}
Projected values at 12 weeks: ${JSON.stringify(predictions.weeks12, null, 2)}
...
Return only the insight paragraph, no markdown, no bullet points.`
    ...
    return res.status(200).json({ success: true, predictions, aiInsight })
  } catch (err) {
    console.error('predict error:', err)
    return res.status(500).json({
      success: false,
      error: err.message || 'An unexpected error occurred while generating predictions.',
    })
  }
}
